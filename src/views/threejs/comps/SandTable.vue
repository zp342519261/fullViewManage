<template>
  <div class="sand-table">
    ...
    <el-input label="图片地址" placeholder="请输入图片地址" v-model="form.url" @change="changeHandle">
    </el-input>
    ...
    <div class="mt-2">
      ...
      <div class="marker-list mt-2">
        <div class="marker-item" v-for="(item, index) in sandTable.markers" :key="index"
          :class="{ 'is-active': index === activeIndex }" @click="clickMarkerHandle(index, item)">
...
export default {
  ...
  methods: {
    ...
    delMarkerHandle(index) {
      const sandTableClone = lodash.cloneDeep(this.sandTable);
